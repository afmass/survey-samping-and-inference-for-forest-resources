---
title: "Model-assisted estimation under the Monte Carlo approach"
subtitle: "From the external model assumption to the classical regression estimator"
...
## Pros and cons of one-phase estimation

\textcolor{ForestGreen}{\textbf{Pros}}

- **Simple and robust:** the sample mean is consistent by the LLN
- **Few assumptions:** only sampling design and error-free measurements are required
- **Straightforward inference:** confidence intervals follow from the CLT
  $$
...
- **Asymptotic:** good approximation requires a sufficiently large \($n$\)
- **Inefficient:** ignores information beyond the sampled \($Y(x)$\)
...
p5 <- ggplot() +
  geom_line(data = local_density, aes(x = x, y = y_scaled, linetype = "Y"), color = "red", linewidth = 1) +
  ...
  scale_linetype_manual(name = "", 
                        values = c("Y" = "solid", "Y_hat" = "solid"),
                        labels = c("Y" = expression(Y(x)), "Y_hat" = expression(hat(Y)(x)))
                        ) +
  geom_segment(aes(x = 0, xend = lambda_F, y = 0, yend = 0), linewidth = 1.5) +
  ylim(min(trees$y), max(trees$height)) +
  ggtitle("Predicted density surface \n (\"known\" integral)") +
  theme_void() +
  guides(linetype = guide_legend(direction = "horizontal", nrow = 1)) +
  theme(plot.title = element_text(hjust = 0.5, size = 20),
        legend.position = c(0.5, 0.1),
        legend.text = element_text(size = 18),
        plot.margin = margin(t = 5, r = 10, b = 10, l = 10))
...
p6 <- ggplot() +
  ...
  ylim(min(trees$y), max(trees$height)) +
  geom_point(data = sampled_plots, aes(x = x, y = 0), shape = 4, size = 3, color = "black", stroke = 1.5) +
  geom_point(data = sampled_plots, aes(x = x, y = y, color = "Observed"), shape = 20, size = 3, stroke = 1.5) +
  ...
  ggtitle(bquote(atop("MC integration on residual",
                    R(x) == Y(x) - hat(Y)(x)))) +
  scale_color_manual(name = "", values = c("Observed" = "red", "Predicted" = "blue")) +
  theme_void() +
  theme(plot.title = element_text(hjust = 0.5, size = 20),
...
- If the model is external, the $\hat{Y}_{diff}$ and variance estimator are \textbf{exactly design-unbiased}.
...
## How do we account for the internal-fit contribution to variance?
\underline{Two common approaches:}
\vspace{0.7em}
...
## Design-based covariance of $\hat{\boldsymbol{\beta}}$

For a \textbf{linear model},
\vspace{-.2em}
...
By a \textbf{first-order Taylor expansion} (see Mandallaz p. 124-125 for derivation),
...
Under uniform iid sampling an asymptotic \textbf{design-based covariance estimate} for $\hat{\beta}_s$,
...
## Classical regression estimator has good statistical properties...

Defined $g(x)=\bar{\mathbf Z}^{\,t}\mathbf A_s^{-1}\mathbf Z(x)$ and re-write
as :
$$
\boxed{
\hat{Y}_{reg}
...
\centering
The g-weighted sample exactly reproduces the known population means in the auxiliary vector. \newline\newline 
\pause
**Great news if $\mathbf Z(x)$ and $Y(x)$ are well-correlated!**

## Post-stratification

\textbf{Post-stratification} : classical regression estimation with categorical variables (i.e. ANOVA).
\vspace{-.15em}
$$
\boxed{
\hat Y_{reg}
...
## Special cases of the classical regression estimator

With **known wall-to-wall auxiliary information**, several important \underline{special cases} arise:
\vspace{.3cm}
...
\vfill
\footnotesize
\textbf{NOTE:} We only need to implement the g-weight variance formula once!
...
## BONUS: a taste of the classical survey sampling theory (Cordy, 1993)
<!-- \begin{block}{Other approaches} -->
<!-- Most people don't use the Monte Carlo approach to learn this material.  This is the classical setup to the problem. -->
<!-- \end{block} -->
\footnotesize
...
- The **sampling intensity** (\textit{inclusion prob.} from finite theory) is
$$
\pi(x) = \sum_{k=1}^n f(x) = n\,f(x) = \frac{n}{A_T}, \qquad x \in T
$$
- The **second-order sampling intensity** (\textit{inclusion prob.} of a pair being jointly sampled) is
$$
